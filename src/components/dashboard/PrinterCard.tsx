import { PrinterWithToners, useSedes, getSedeNome } from "@/hooks/usePrinterData";
    ...
    >
      ...
      <div className="space-y-2">
        <p className="text-xs font-medium text-muted-foreground uppercase tracking-wider">
          Toners {isColor ? "(CMYK)" : "(Mono)"}
        </p>
        <TonerBar color="black" level={printer.toners.black} />
        {isColor && (
          <>
            <TonerBar color="cyan" level={printer.toners.cyan ?? 0} />
            <TonerBar color="magenta" level={printer.toners.magenta ?? 0} />
            <TonerBar color="yellow" level={printer.toners.yellow ?? 0} />
          </>
        )}
      </div>
    </div>
